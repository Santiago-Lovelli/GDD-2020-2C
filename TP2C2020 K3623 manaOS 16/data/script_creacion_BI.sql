...
ALTER TABLE GD2C2020.manaOS_BI.HechosFactura
   ADD CONSTRAINT FK_Factura_TIEMPO FOREIGN KEY (TIEMPO_ID)
      REFERENCES GD2C2020.manaOS_BI.Tiempo(TIEMPO_CODIGO)
      ON DELETE CASCADE
      ON UPDATE CASCADE
;
...
ALTER TABLE GD2C2020.manaOS_BI.HechosCompra
   ADD CONSTRAINT FK_Compra_TIEMPO FOREIGN KEY (TIEMPO_ID)
      REFERENCES GD2C2020.manaOS_BI.Tiempo(TIEMPO_CODIGO)
      ON DELETE NO ACTION
      ON UPDATE NO ACTION
;
...
-- HechosFactura
INSERT INTO [GD2C2020].[manaOS_BI].[HechosFactura] ([FACTURA_NRO],[CANT_FACTURADA],[CLIENTE_ID],[TIEMPO_ID],[PRECIO_FACTURADO],[SUCURSAL_ID],[PRECIO_AUTO_FACTURADO],[AUTO_ID],[AUTOPARTE_ID],[PRECIO_AUTOPARTE_FACTURADO])
	SELECT f.[FACTURA_NRO],f.[CANT_FACTURADA],f.[CLIENTE_ID],t.[TIEMPO_CODIGO],f.[PRECIO_FACTURADO],f.[SUCURSAL_ID] ,fau.[PRECIO_AUTO_FACTURADO],fau.[AUTO_ID],faup.[AUTOPARTE_ID],faup.[PRECIO_AUTOPARTE_FACTURADO]
...
-- HechosCompra
INSERT INTO [GD2C2020].[manaOS_BI].[HechosCompra] ([COMPRA_CANT],[TIEMPO_ID],[COMPRA_NRO],[COMPRA_PRECIO],[FACTURA_NRO],[AUTO_ID],[COMPRA_AUTO_PRECIO],[AUTOPARTE_ID],[COMPRA_AUTOPARTE_PRECIO])
	SELECT c.[COMPRA_CANT],t.[TIEMPO_CODIGO], c.[COMPRA_NRO],c.[COMPRA_PRECIO],c.[FACTURA_NRO], cau.[AUTO_ID],cau.[COMPRA_AUTO_PRECIO], caup.[AUTOPARTE_ID],caup.[COMPRA_AUTOPARTE_PRECIO]
	...
	INNER JOIN [manaOS_BI].[Tiempo] t ON t.MES_NRO = MONTH(c.COMPRA_FECHA) AND t.ANIO_NRO = YEAR(c.COMPRA_FECHA)
...
CREATE VIEW [manaOS_BI].vista_automoviles_vendido_x_sucursal_y_mes AS 
	...
	COUNT(a.[AUTO_ID]) AS CANTIDAD_DE_AUTOMOVILES, t.[MES_NRO] AS MES
	...
	INNER JOIN [GD2C2020].[manaOS_BI].[Tiempo] t ON f.[TIEMPO_ID] = t.[TIEMPO_CODIGO]
	GROUP BY s.[SUCURSAL_DIRECCION],s.[CIUDAD_NOMBRE], t.[MES_NRO]
GO
...
CREATE VIEW [manaOS_BI].vista_ganancias_xSucursal_xMes_AUTO AS 
	...
	INNER JOIN [manaOS_BI].[Tiempo] t ON t.[TIEMPO_CODIGO] = f.[TIEMPO_ID]
...
CREATE VIEW [manaOS_BI].vista_ganancias_xSucursal_xMes_AUTOPARTE AS 
	...
	INNER JOIN [manaOS_BI].[Tiempo] t ON t.[TIEMPO_CODIGO] = fa.[TIEMPO_ID] AND t.[TIEMPO_CODIGO] = c.[TIEMPO_ID]
